...
class FlowController extends CommonController 
{
    ...
    public function index()
    {
        ...
        if ($auid == 1) {
            ...
            // 搜索功能
            $map = array(
                'orderid' => array('like','%'.trim(I('post.orderid')).'%'),
                'f.device_code' => array('like','%'.trim(I('post.device_code')).'%'),
            ...
            //     $map['money'][] = array('elt',$maxmoney*100);
            // }
            // if (is_numeric($maxmoney )) {
            //     $map['money'][] = array('egt',$minmoney*100);
            // }
        ...
        if (IS_POST) {
            ...
            if(!empty($_SESSION['adminuser'])){
                ...
                switch ($case) {
                    ...
                    case '0':
                        $user = D('adminuser')->getAll();
                        break;

                    case '1':
                    // 一级经销商只能给二级经销商绑定设备，如需再往下分级，则case '2' leavel=3
                        $user = M('adminuser')->where('leavel=2')->select();
    ...
    public function bindinglist()
    {
        ...
        $map = array(
            ...
            'pub_adminuser.phone' => array('like','%'.trim(I('post.phone')).'%'),
            'pub_adminuser.name' => array('like','%'.trim(I('post.name')).'%'),
        );
        if($this->get_level()){
            $map['pub_adminuser.id'] = $_SESSION['adminuser']['id'];
        }
         $minaddtime = strtotime(trim(I('post.minaddtime')))?:0;
         $maxaddtime = strtotime(trim(I('post.maxaddtime')))?:-1;
         if (is_numeric($maxaddtime)) {
        ...
        if (I('output') == 1) {
            $data = $binding->where($map)
                ->join('pub_adminuser ON pub_binding.vid = pub_adminuser.id')
                ->join('pub_devices ON pub_binding.did = pub_devices.id')
                ->field('pub_adminuser.id,pub_binding.did,pub_devices.device_code,pub_adminuser.name,pub_adminuser.phone,pub_binding.addtime')
        ...
        $total =$binding->where($map)
                    ->join('pub_adminuser ON pub_binding.vid = pub_adminuser.id')
                    ->join('pub_devices ON pub_binding.did = pub_devices.id')
                    ->field('pub_binding.*,pub_adminuser.name,pub_adminuser.phone,pub_devices.device_code')
                    ->count();
        $page  = new \Think\Page($total,8);
        $pageButton =$page->show();
